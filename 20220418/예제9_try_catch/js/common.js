...
class User {
  constructor( name, age, local ) {
    ...
    try {
      ...
      // throw new Error("헉!! 이런 오류!!");  // 예외처리 : 강제 에러 발생
      boxCheck(); // 존재하지 않는 함수를 임의로 호출하여 오류를 발생시킴
    } catch (e) {
...
// } catch (e) {
//    // 이 블록 내부의 문장들 오직 try 블록에서 예외가 발생할 경우에만 실행된다.
//    // 이 문장들에서는 지역 변수 e를 사용하여 Error 객체 또는 앞에서 던진 다른값을 참조할 수 있다.
